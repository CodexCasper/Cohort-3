  ...
  return (
    ...
    {/*  <a href='/'>Allen</a> | <a href='/neet/online-coaching-class-11'>11th</a> | <a href='/neet/online-coaching-class-12'>12th</a>       {/* Navigating thorugh each content, through naive way but it will re-load the whole page for every click on these pages   */}
      ...
      {/* we must import link from react and then same as anchor tag we must use link to directly naviagate without reloading whole page, NOTE: Link must be declare inside BrowserRouting only  */}
         ...
         <Routes>  
                                                                                            {/* and inside BrowserRouter we must define our routes ( routes => means whate\ever comes after / , i.e. http://localhost:300/"ROUTES" ) */}
...
function Layout(){
  return <div style={{ height:"100vh"}}>
     <Link to="/">Allen</Link> 
      ...
      <div style={{ height:"90vh"}}> {/*vh means it covers 90 % of the page */}
         <Outlet />                  {/*what this outlet does is whenever we define superRoute for ChildrenRoutes then first the Header is rendered then all the routes in b/w gets rendered & in the last the footer  */}
      </div>

    Footer
  </div>
}
//function component to display if the route is not correct 
function NoPage(){
  return <div>
    Sorry page not found !
  </div>
}
// A fucntional component created for class 11th route 
