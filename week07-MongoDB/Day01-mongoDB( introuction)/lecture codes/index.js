...
//IMPORTING MODELS IN js that we have creadted in DB.js
...
mongoose.connect("mongodb+srv://parthbindal5:NoExitparth@cluster0.nbytjoo.mongodb.net/TODO_APP_DATABASE");          //how will it know where to store, so it connects ous with mongo 
...
app.post("/todo",auth, async function(req,res){             //jese hi middleware sab kuch pass kr dega so the control will reach over here in todo and todos routes 
    ...
    await TodoModel.create({               //database call isliye await kiya
...
function auth(req,res,next){                                //authentication middleware to check if the user is logged in or not
    ...
    const decodedToken = jwt.verify(token,JWT_SECRET)                  // ater decoding we must verify token and store in a var

    if(decodedToken){                                                  //if decodedToken is true then
        req.userId = token.id;                                      //req.userId me store kara kyuki req is common and sab routes ek hi req use krte hai so that we can acces it directly in our authenticated endpoints(todos,Todo)
