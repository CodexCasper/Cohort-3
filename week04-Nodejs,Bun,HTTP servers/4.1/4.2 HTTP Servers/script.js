...
HEADER[ metadata (info about request/response) ] => CONTAINS 
1.request url == where the request(sended by client) goes to the server 
2.request method == GET , PUT , DELETE ,CREATE
3.status code == 200, 404 
4.remote adress ==142.250.193.228:443 , is the real address of the server

"RESPONSE" => is a raw response server gives to us  i.e HTML page , JSON , image 
"preview "=> we can see what does the raw repsonse look like , bassically browser-friendly view of response (same data , formatted nicely)
*/

// why HTTP protocol => HTTP allows client and servers to communicate over the internet in a standard way 

// Request Response Model => is just a kind of communication pattern bw client
// once cycle:
// 1. -> ( client -> SEND REQUEST -> asks for data ) 
// 2. -> server ( processes )
// 3. -> server( sends reponses )
// 4. -> connection ends
// others protocols are websockets , webrtc , grpc
// NOTE: HTTP is stateless , which means it do not remember previous requests 
...
a way to reach a server is through its " DOMAIN NAME "  
...
// "ports" - are logical endpoints('doors') used by protocols to identify a specific processes running on a computer/server , helps direct traffic to correct application or service on a system   
// i.e 152.42.156.126:444 -> machine(IP) , which app/service  ->  '444' 
// common one includes -> 80 , HTTP
// 443 , HTTPS 
// 3000 , dev servers (react, node) 
...
//interview queston -> PUT  -> replace full data vs PATCH -> updates partial data 
// 

// "RESPONSE" - WHAT the server responds you to in response to the request
// types 
// 1.PLAINTEXT 
// 2.HTML => if it is a website( i.e. google,facebook) 
// 3.JSON( JAVASCRIPT OBJECT NOTATION ) - if we want to fetch some data and once we get it we can render it 
...
//    ports vs routes
// select which app/service  vs select what action inside app
//i.e :3000   vs  /todos  